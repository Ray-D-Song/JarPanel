...
const message = {
  ...
  container: {
    ...
    cutLogHelper3: '注意，重新创建容器可能会导致数据丢失。如果你的容器中有重要数据，确保在执行重建操作之前进行备份。',
    maxSize: '文件大小',
    maxFile: '保留份数',
  ...
  toolbox: {
    ...
    clam: {
      ...
      showFresh: '显示病毒库服务',
      hideFresh: '隐藏病毒库服务',
      clamHelper: 'ClamAV 的最低建议配置为：3 GiB 以上的 RAM，2.0 GHz 以上的单核 CPU，以及至少 5 GiB 的可用硬盘空间',
      doc: '帮助文档',
  ...
  file: {
    ...
    ignoreCertificateHelper: '下载时忽略不可信证书可能导致数据泄露或篡改。请谨慎使用此选项，仅在信任下载源的情况下启用',
  ...
  website: {
    ...
    websiteProxyHelper: '代理已有服务,例如本机已安装使用 8080 端口的 halo 服务，那么代理地址为 http://127.0.0.1:8080',
  ...
  ssl: {
    ...
    shell: '脚本内容',
    shellHelper: '脚本默认执行目录为 1Panel 安装目录，如果有推送证书，那么执行目录为证书推送目录。默认超时时间 30 分钟',
  },
  firewall: {
    create: '创建规则',
    edit: '编辑规则',
