...
const message = {
  ...
  container: {
    ...
    cutLogHelper3: '註意，重新創建容器可能會導致數據丟失。如果你的容器中有重要數據，確保在執行重建操作之前進行備份。',
    maxSize: '文件大小',
    maxFile: '保留份數',
  ...
  toolbox: {
    ...
    clam: {
      ...
      showFresh: '顯示病毒庫服務',
      hideFresh: '隱藏病毒庫服務',
      clamHelper: 'ClamAV 的最低建議配置為：3 GiB 以上的 RAM，2.0 GHz 以上的單核 CPU，以及至少 5 GiB 的可用硬盤空間。',
  ...
  file: {
    ...
    ignoreCertificateHelper: '下載時忽略不可信證書可能導致數據洩露或篡改。請謹慎使用此選項，僅在信任下載源的情況下啟用',
  ...
  website: {
    ...
    websiteProxyHelper: '代理已有服務,例如本機已安裝使用 8080 端口的 halo 服務，那麼代理地址為 http://127.0.0.1:8080',
  ...
  ssl: {
    ...
    shell: '腳本內容',
    shellHelper: '腳本預設執行目錄為 1Panel 安裝目錄，如果有推送證書，那麼執行目錄為證書推送目錄。預設超時時間 30 分鐘',
  },
  firewall: {
    create: '創建規則',
    edit: '編輯規則',
