  ...
  >
    ...
    <div>
      ...
      <BreadCrumbs>
        ...
        <template v-if="paths.length > 2">
          <BreadCrumbItem>
            <el-dropdown ref="dropdown1" trigger="click" @command="jump($event)">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item v-for="(item, key) in paths.slice(0, -1)" :key="key" :command="key">
                    {{ item }}
                  </el-dropdown-item>
                </el-dropdown-menu>
              </template>
        ...
        <template v-else>
          <BreadCrumbItem v-for="(item, key) in paths" :key="key" @click="jump(key)" :right="key == paths.length - 1">
    ...
    <div>
      <el-table :data="data" highlight-current-row height="40vh">
        ...
        <el-table-column show-overflow-tooltip fix>
          <template #default="{ row }">
            <div>
              <svg-icon :class="'table-icon'" :iconName="row.isDir ? 'p-file-folder' : 'p-file-normal'"></svg-icon>
