    ...
    <div class="app-status" v-if="data.isExist">
      <el-card>
        ...
        <div v-if="showFresh" class="flex w-full flex-col gap-4 md:flex-row mt-5">
          ...
          <div class="mt-0.5">
            <el-button type="primary" v-if="!data.freshIsActive" link @click="onOperate('FreshClam', 'start')">
              {{ $t('app.start') }}
            </el-button>
            <el-button type="primary" v-if="data.freshIsActive" link @click="onOperate('FreshClam', 'stop')">
              {{ $t('app.stop') }}
            </el-button>
            <el-divider direction="vertical" />
