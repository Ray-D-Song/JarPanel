    ...
    <LayoutContent :title="$t('container.image')" :class="{ mask: dockerStatus != 'Running' }">
      ...
      <template #main>
        <ComplexTable :pagination-config="paginationConfig" :data="data" @search="search">
          ...
          <el-table-column :label="$t('container.tag')" prop="tags" min-width="160" :width="mobile ? 400 : 'auto'" fix>
            <template #default="{ row }">
              <el-tag class="ml-2.5" v-for="(item, index) of row.tags" :key="index" :title="item" type="info">
                {{ item }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column :label="$t('container.size')" prop="size" min-width="60" fix />
          <el-table-column prop="createdAt" min-width="80" :label="$t('commons.table.date')" :formatter="dateFormat" />
          <fu-table-operations width="250px" :ellipsis="10" :buttons="buttons" :label="$t('commons.table.operate')" />
        </ComplexTable>
      </template>
    </LayoutContent>
