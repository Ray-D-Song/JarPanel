    ...
    >
      ...
      <el-table :data="data" class="mt-5" @selection-change="handleSelectionChange">
        ...
        <el-table-column :label="$t('terminal.quickCommand')" min-width="120" show-overflow-tooltip>
          <template #default="{ row }">
            <el-input v-if="row.lineStatus === 'create' || row.lineStatus === 'edit'" v-model="row.command" />
            <span v-else>{{ row.command }}</span>
          </template>
        </el-table-column>
        <el-table-column min-width="40">
          <template #default="scope">
            <el-button v-if="scope.row.lineStatus === 'create'" link type="primary" @click="handleCmdSave(scope.row)">
