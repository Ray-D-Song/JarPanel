        ...
        <el-col :span="22">
          <el-form-item :label="$t('cronjob.taskType')" prop="type">
            ...
            <div v-if="dialogData.rowData!.type === 'ntp'">
              <span class="input-help">
                ...
                <el-link style="font-size: 12px" icon="Position" @click="goRouter('/toolbox/device')" type="primary">
                  {{ $t('firewall.quickJump') }}
                </el-link>
              </span>
            </div>
          ...
          <el-form-item :label="$t('cronjob.cronSpec')" prop="spec">
            <div v-for="(specObj, index) of dialogData.rowData.specObjs" :key="index" style="width: 100%">
              ...
              <el-input v-if="specObj.specType !== 'perNSecond'" class="specClass" v-model.number="specObj.minute">
                <template #append>
                  <div class="append">{{ $t('commons.units.minute') }}</div>
                </template>
              </el-input>
              <el-input v-if="specObj.specType === 'perNSecond'" class="specClass" v-model.number="specObj.second">
                <template #append>
          ...
          >
            <el-input v-model="dialogData.rowData!.sourceDir">
              <template #prepend>
                <FileList @choose="loadDir" :dir="true" :path="dialogData.rowData!.sourceDir"></FileList>
              </template>
            </el-input>
          </el-form-item>
...
const verifySpec = (rule: any, value: any, callback: any) => {
  ...
  for (let i = 0; i < dialogData.value.rowData!.specObjs.length; i++) {
    ...
    switch (item.specType) {
      case 'perMonth':
        if (item.day < 0 || item.day > 31 || item.hour < 0 || item.hour > 23 || item.minute < 0 || item.minute > 59) {
          callback(new Error(i18n.global.t('cronjob.cronSpecRule', [i + 1])));
          return;
        }
        break;
      case 'perNDay':
        if (item.day < 0 || item.day > 366 || item.hour < 0 || item.hour > 23 || item.minute < 0 || item.minute > 59) {
          callback(new Error(i18n.global.t('cronjob.cronSpecRule', [i + 1])));
          return;
        }
        break;
      case 'perWeek':
        if (item.week < 0 || item.week > 6 || item.hour < 0 || item.hour > 23 || item.minute < 0 || item.minute > 59) {
          callback(new Error(i18n.global.t('cronjob.cronSpecRule', [i + 1])));
          return;
        }
        break;
